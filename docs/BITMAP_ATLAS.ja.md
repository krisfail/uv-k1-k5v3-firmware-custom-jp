...
特大字形は同じ16行契約で横幅を10列へ拡張しています．`UI_PrintStringJapaneseExtraLarge`は，特大表へ登録された日本語コードだけを指定位置へ描画する専用APIです．K1では公開フォントの線構造を独立に変換・再構成した「受・信・専・用」の10×16セル4スロットを，受信専用の起動画面に使用します．出所と変換規則は[FONT_SOURCES.ja.md](FONT_SOURCES.ja.md)を参照してください．
