...
v5.9.0への更新で追加された機能のうち，受信運用とUIに直接役立つものは`JpRxOnly`で有効にしています．FM放送スキャン中に通常チャンネルの受信処理が割り込まない制御，メモリースキャンのチャンネル名表示中に再開タイマーを保留する制御，カテゴリ画面の`ALL`フォールバック，カテゴリ項目数の2桁表示，`F`＋サイドキー長押しの操作選択が対象です．

アクション選択は受信操作（モニター，スキャン，FM，VFO切替，帯域幅など）だけを候補にし，RX専用版では`POWER`と`PTT`を表示しません．古い保存設定に送信系アクション番号が残っていても，実行側で拒否します．Fox Hunt／Beacon，BYP／BYP+，送信タイマー，送信電力・送信トーンを伴う変更は，コードが上流から取り込まれていても本版では有効化しません．
