...
起動画面で使う`受`，`信`，`専`，`用`の10×16特大字形と，`0x98=専`，`0x99=用`の通常大字形を，Unifoundryが公開する**Izumi 16 Plane 1，JIS X 0213:2004**の16×16 BDFから独立に変換しました．公式ページは，このIzumi 16をパブリックドメインのJIS X 0213フォントとして説明しています．
...
K1では通常大字形の`0x98`／`0x99`と，`ALL`／`MESSAGE`で使う10×16特大字形の4字を更新しました．`専`／`用`は通常大字形・特大字形とも独立の低解像度再構成を使い，細い線の消失と字幅の崩れを抑えています．ビルドは取得済みBDFを必要とせず，変換後のC配列だけを使用します．
