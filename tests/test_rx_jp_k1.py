...
class K1ReceiveOnlyStaticTests(unittest.TestCase):
    ...
    def test_k1_receive_bandwidth_has_four_modes_and_preserves_step_choices(self):
        ui = source("App/ui/main.c")
        action = source("App/app/action.c")
        radio = source("App/radio.c")
        menu = source("App/ui/menu.c")
        ...
        self.assertIn('"W+", "W", "N", "N-"', ui)
        self.assertIn('"W+",\n    "W",\n    "N",\n    "N-"', menu)
        self.assertIn("RADIO_NextBandwidth", action)
        self.assertIn("RADIO_BandwidthFromMenuIndex", app_menu)
        self.assertIn("RADIO_BandwidthToMenuIndex", app_menu)
        self.assertIn("BANDWIDTH_WIDE_PLUS", action)
