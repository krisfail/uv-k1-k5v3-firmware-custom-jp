...
メニューの`受信拡張`（従来表記`RXExt`）を`OFF`にすると、プリセット、WIDE+、受信モードの`SINGLE`、メモリーバンク絞り込み、`自動`スケルチ、AGCガード、一時スキップを停止します。受信専用・PTTモニター、日本語表示、FM放送の`76.0–95.0 MHz`制限は変わりません。既存の保存データは互換性のためONとして扱います。
