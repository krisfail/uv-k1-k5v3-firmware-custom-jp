...
def parse_source(path: Path, occurrences: dict[str, int]) -> list[Array]:
    ...
    for match in DECL_RE.finditer(text):
        ...
        arrays.append(Array(match.group("name"), path.as_posix(), dimensions, values,
                            occurrences[match.group("name")], width,
                            parse_glyph_annotations(match.group("name"), initializer, width)))
    return arrays
...
def main() -> None:
    ...
    for array in arrays:
        layout = array.glyph_layout
        record = {"name": array.label, "category": array.category, "source": array.source, "dimensions": array.dimensions, "element_width": array.element_width, "size": len(array.values), "bytes_hex": array.values.hex(" "), "layout": "glyph-2page" if layout else "column-strip"}
        if layout:
    ...
    (out / "bitmap_atlas_inventory.json").write_text(json.dumps({"sources": [str(p.resolve()) for p in sources], "arrays": inventory}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
